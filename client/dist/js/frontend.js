...
var FluxAttrs;
(function (FluxAttrs) {
  FluxAttrs["EVENT"] = "fx-event";
  FluxAttrs["ACTION"] = "fx-action";
})(FluxAttrs || (FluxAttrs = {}));
...
const updateDOMTemplate = (event, newDOMContent) => {
  if (!event.target) return;
  // @TODO include TrustedTypes API createNew check
  const sanitizedHTML = dompurify_1.default.sanitize(newDOMContent);
...
const fetchEvent = event => {
  ...
  fetch(`${API_ENDPOINT}/${event.endpoint}`).then(res => {
 ...
 */
const mapEventToDOM = fluxEvent => {
  const target = document.querySelector(fluxEvent.target);
...
/******/ 	var __webpack_exports__ = __webpack_require__("./client/core/index.ts");
/******/ 	
/******/ })()
;
